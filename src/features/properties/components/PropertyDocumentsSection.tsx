...
function DocumentRow({ doc, onRemoved }: DocumentRowProps) {
  ...
  const handleRemove = async () => {
    ...
    setError(null);
    try {
      await removePropertyDocument(doc.id);
      onRemoved();
    } catch (err) {
  ...
  return (
    <li className="property-document-row">
      <span className="property-document-row__icon" aria-hidden="true">
        <FileText size={20} strokeWidth={1.75} />
      </span>
      <span className="property-document-row__text">
        <span className="property-document-row__title">{doc.title}</span>
        <span className="property-document-row__meta">
      ...
      <span className="property-document-row__actions">
...
export function PropertyDocumentsSection({ documents, onAdd, onRefresh }: PropertyDocumentsSectionProps) {
  const { t } = useLanguage();

  return (
    <div className="property-documents-section">
      {documents.length === 0 ? (
        <p className="property-documents-section__empty">{t('documentsEmpty')}</p>
      ) : (
        <ul className="property-documents-section__list">
          {documents.map((doc) => (
