...
import type { StaffDocumentFormValues, StaffDocumentType } from '../types';
import { DOCUMENT_FILE_INPUT_ACCEPT, validateDocumentFile } from '../validation';
import './StaffDocumentForm.css';

interface StaffDocumentFormProps {
  onSubmit: (values: StaffDocumentFormValues) => Promise<void>;
  onCancel: () => void;
}

export function StaffDocumentForm({ onSubmit, onCancel }: StaffDocumentFormProps) {
  const { t, locale } = useLanguage();
  const formId = useId();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [type, setType] = useState<StaffDocumentType>('civilId');
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setSubmitError(null);
    try {
      const typeLabel = STAFF_DOCUMENT_TYPES.find((docType) => docType.id === type)?.title[locale] ?? '';
      await onSubmit({
        type,
  ...
  return (
    <form className="staff-document-form" onSubmit={handleSubmit} noValidate>
      <FormField label={t('fieldDocumentType')} htmlFor={`${formId}-type`}>
        ...
        >
          {STAFF_DOCUMENT_TYPES.map((docType) => (
      ...
      <FormField label={t('fieldDocumentFile')} htmlFor={`${formId}-file`} error={fileError ?? undefined}>
        ...
        <input
          ref={fileInputRef}
          id={`${formId}-file`}
          className="staff-document-form__file-input"
          type="file"
      ...
      {submitError && (
        <p className="staff-document-form__error" role="alert">
          {submitError}
        </p>
      )}

      <div className="staff-document-form__actions">
