...
function DocumentRow({ doc, onRemoved }: DocumentRowProps) {
  ...
  const handleRemove = async () => {
    ...
    setError(null);
    try {
      await removeFamilyMemberDocument(doc.id);
      onRemoved();
    } catch (err) {
  ...
  return (
    <li className="member-document-row">
      <span className="member-document-row__icon" aria-hidden="true">
        <FileText size={20} strokeWidth={1.75} />
      </span>
      <span className="member-document-row__text">
        <span className="member-document-row__title">{doc.title}</span>
        <span className="member-document-row__meta">
      ...
      <span className="member-document-row__actions">
...
export function MemberDocumentsSection({ documents, onAdd, onRefresh }: MemberDocumentsSectionProps) {
  const { t } = useLanguage();

  return (
    <div className="member-documents-section">
      {documents.length === 0 ? (
        <p className="member-documents-section__empty">{t('documentsEmpty')}</p>
      ) : (
        <ul className="member-documents-section__list">
          {documents.map((doc) => (
